...
class NamBackGestureDetector<T> extends StatefulWidget {
  const NamBackGestureDetector({
    super.key,
  ...
  @override
  NamBackGestureDetectorState<T> createState() => NamBackGestureDetectorState<T>();
}

class NamBackGestureDetectorState<T> extends State<NamBackGestureDetector<T>> {
  ...
  double _dragAreaWidth(BuildContext context) {
    ...
    // that has the notch.
    final dragAreaWidth = Directionality.of(context) == TextDirection.ltr ? context.padding.left : context.padding.right;
...
mixin NamPageRouteTransitionMixin<T> on PageRoute<T> {
...
Cannot read the previousTitle for a route that has not yet been installed''',
  ...
  bool canTransitionTo(TransitionRoute<dynamic> nextRoute) {
    ...
    // fullscreen dialog.
    return (nextRoute is NamPageRouteTransitionMixin && !nextRoute.fullscreenDialog && nextRoute.showCupertinoParallax) || (nextRoute is CupertinoRouteTransitionMixin && !nextRoute.fullscreenDialog);
  }
  ...
  static Widget buildPageTransitions<T>(
    ...
    final route = rawRoute as NamPackPageRoute<T>;
    final linearTransition = route.popGestureInProgress;
    final finalCurve = route.curve ?? Curves.easeOutQuad;
    ...
    if (route.fullscreenDialog && route.transition == null) {
      ...
      final canSwipe = route.popGesture;
      if (route.customTransition != null) {
        return route.customTransition!.buildTransition(
          ...
          animation,
          secondaryAnimation,
          NamBackGestureDetector<T>(
            popGestureEnable: () => _isPopGestureEnabled(route, canSwipe, context),
            onStartPopGesture: () {
              assert(_isPopGestureEnabled(route, canSwipe, context));
      ...
      switch (route.transition ?? Transition.native) {
        ...
        case Transition.noTransition:
          return NamBackGestureDetector<T>(
            popGestureEnable: () => _isPopGestureEnabled(route, canSwipe, context),
            onStartPopGesture: () {
              assert(_isPopGestureEnabled(route, canSwipe, context));
              return _startPopGesture(route);
        ...
        case Transition.cupertino:
          return CupertinoPageTransition(
              ...
              secondaryRouteAnimation: secondaryAnimation,
              linearTransition: linearTransition,
              child: NamBackGestureDetector<T>(
                popGestureEnable: () => _isPopGestureEnabled(route, canSwipe, context),
                onStartPopGesture: () {
                  assert(_isPopGestureEnabled(route, canSwipe, context));
                  return _startPopGesture(route);
        ...
        default:
          ...
          return pageTransitionsTheme.buildTransitions(
              ...
              NamBackGestureDetector<T>(
                ...
                onStartPopGesture: () {
                  assert(
                    _isPopGestureEnabled(route, canSwipe, context),
                  );
                  return _startPopGesture(route);
                },
                limitedSwipe: limitedSwipe,
  ...
  // Called by NamBackGestureDetector when a pop ("back") drag start
